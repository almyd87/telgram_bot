...
# الأزواج المتاحة (من Pocket Option OTC)
SYMBOLS = {
    "EUR/USD": "EURUSD=X",
    "GBP/USD": "GBPUSD=X",
    "USD/JPY": "USDJPY=X",
...
# لتخزين اختيارات المستخدم
user_selections = {}

# التحليل باستخدام RSI + Bollinger Bands + MA
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [InlineKeyboardButton(symbol, callback_data=f'symbol_{symbol}')] for symbol in SYMBOLS.keys()
    ]
...
async def handle_symbol(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(duration, callback_data=f'duration_{duration}')] for duration in DURATIONS
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
